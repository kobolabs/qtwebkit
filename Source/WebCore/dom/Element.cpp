...
void Element::recalcStyle(StyleChange change)
{
    ...
    if (hasParentStyle && (change >= Inherit || needsStyleRecalc())) {
        ...
        if (RenderObject* renderer = this->renderer()) {
            if (!currentStyle.get()->maxHeight().isUndefined() && (localChange != NoChange || pseudoStyleCacheIsInvalid(currentStyle.get(), newStyle.get()) || (change == Force && renderer->requiresForcedStyleRecalcPropagation()) || styleChangeType() == SyntheticStyleChange))
